...
static alpha(dt,cutoff){const tau=1.0/(2Math.PIcutoff);return 1.0/(1.0+tau/dt);}
filterVec(t,x){
...
const dxHat=aDdx+(1-aD)this.dxPrev;
const cutoff=this.minCutoff+this.betaMath.abs(dxHat);
const aX=OneEuro.alpha(dt,cutoff);
const xHat=aXx+(1-aX)this.xPrev;
...
/ ===========================
MediaPipe model setup (HEAVY MODEL WITH LOW THRESHOLDS)
=========================== /
let pose=null;
...
baseOptions:{
delegate:'CPU', // GPU not supported in web JS, sticking with CPU
modelAssetPath:
...
outputSegmentationMasks:true // Enabled for potential better accuracy in partial views
});
console.log('Pose model initialized with HEAVY model and LOW thresholds (CPU)');
}
/ ===========================
Colors
=========================== /
...
if(!L)return;
const toPx=(p)=>({x:p.xcanvas.width,y:p.ycanvas.height});
...
for(const p of[LS,RS,LH,RH,LK,RK]){ctx.beginPath();ctx.arc(p.x,p.y,4,0,Math.PI2);ctx.fill();}
}
/* ===========================
...
console.log('No landmarks detected at time:', video.currentTime, ' - Check video content for full body visibility.');
dot.className='status-dot dot-inactive'; // Optional: add a class for inactive status
}
}
